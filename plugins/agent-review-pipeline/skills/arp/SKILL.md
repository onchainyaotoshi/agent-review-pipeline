...
Gemini: gemini-3-flash-preview (orchestrator pinned) | settings.json default: <read at dispatch time>
===============================
```

If auto-fix enabled (`dryRun=false`), change to:
```
Mode: auto-fix (will apply edits, bounded loop)
...
3. **Dependency precheck** (fail fast before any dispatch):
   ...
   - If `both` or `gemini`:
     ...
     - **Cost guard: verify `settings.json` default model matches the ARP pin.** ARP calls the orchestrator with `-m "gemini-3-flash-preview"`, but `/ce:review` spawns persona sub-agents that fall through to `~/.gemini/settings.json` `.model.name` when the spawner does not re-propagate `-m`. If the default is Pro (or Flash-Lite, or any other model), every persona + retry burns that tier's quota while the ARP header shows "Flash pinned" — the cost delta is invisible until the billing page. Empirically on 2026-04-16 a single `/arp 264` with settings.json default `gemini-3.1-pro-preview` hit the 2M paid-tier input-token ceiling in one run (~15 personas × dense context × 5 quota-retries). Guard: `DEFAULT_MODEL=$(jq -r '.model.name // ""' ~/.gemini/settings.json 2>/dev/null)`. If `DEFAULT_MODEL` is non-empty AND != `"gemini-3-flash-preview"`, abort with *"~/.gemini/settings.json default model is '<current>' — /ce:review spawns sub-agents that fall through to this default and will burn <current>'s quota. Update via: jq '.model.name = \"gemini-3-flash-preview\"' ~/.gemini/settings.json > /tmp/s && mv /tmp/s ~/.gemini/settings.json. Re-run /arp afterward."* Empty default (no `model` key) → allow with a soft warn; users who haven't set anything get Google's built-in default which tracks Flash.
